...
bool (*AlternateFunc_CheckTxFeeAndColor)(const CTransaction tx) = NULL;
...
bool IsStandardTx(const CTransaction& tx, string& reason)
{
    ...
    BOOST_FOREACH(const CTxIn& txin, tx.vin) {
        // Biggest 'standard' txin is a 100-of-100 P2SH multisig with compressed
        // keys. (remember the 520 byte limit on redeemScript size) That works
        // out to a (2+100*(33+1))+3=3405 byte redeemScript, 3405+1+100*(73+1)+3=10809
        // bytes of scriptSig, which we round off to 11000 bytes for some minor
        // future-proofing. That's also enough to spend a 100-of-100
        // CHECKMULTISIG scriptPubKey, though such a scriptPubKey is not
        // considered standard)
        if (txin.scriptSig.size() > 11000) {
...
bool AreInputsStandard(const CTransaction& tx, const CCoinsViewCache& mapInputs)
{
    ...
    for (unsigned int i = 0; i < tx.vin.size(); i++)
    {
        ...
        if (!Solver(prevScript, whichType, vSolutions)) {
            LogPrintf("%s : Solver fail\n", __func__);
            return false;
        }
        int nArgsExpected = ScriptSigArgsExpected(whichType, vSolutions);
        if (nArgsExpected < 0) {
            LogPrintf("%s : nArgsExpected >= 0\n", __func__);
            return false;
        }
        ...
        if (!EvalScript(stack, tx.vin[i].scriptSig, SCRIPT_VERIFY_NONE, BaseSignatureChecker())) {
            LogPrintf("%s : EvalScript fail\n", __func__);
            return false;
        }

        if (whichType == TX_SCRIPTHASH) {
            if (stack.empty()) {
                LogPrintf("%s : stack empty\n", __func__);
                return false;
            }
        ...
        if (stack.size() != (unsigned int)nArgsExpected) {
            LogPrintf("%s : stack size != nArgsExpected\n", __func__);
            return false;
        }
    }
...
// Handler for different types of transactions.
...
/*!
 * @brief   Class for the basic transaction handler functions.
 */
...
public:
                    ...
                    const CBlock *pblock)
    {
        ...
        // for mint, output size must be 1
        if (tx.vout.size() != 1) {
                    ...
                    const CBlock *pblock)
    {
        ...
        if (plicense->HasColorOwner(color)) {
            if (tx.vout.size() == 1) {
                ...
                return RejectInvalidTypeTx(
                        "Invalid output amount of LICENSE", state, 100);
        } else {
...
public:
                    ...
                    const CBlock *pblock)
    {
        ...
        if (pblock && (*pblock).GetHash() != consensusParams.hashGenesisBlock) {
            ...
            if (txinfo.GetTxType() != VOTE) {
                return RejectInvalidTypeTx("type of intput must be VOTE", state, 100);
            }
        }

        string candidatescript = tx.vout[0].scriptPubKey.ToString();
        set<string> alliance;
        if (!RedeemScriptToPubkey(candidatescript, alliance, chainParams.AllianceThreshold())) {
            return RejectInvalidTypeTx("Invalid script", state, 100);
        }
        if (alliance.empty()) {
            return RejectInvalidTypeTx("At least 1 pubkey", state, 100);
        }
        return true;
    }
    ...
    bool Apply(const CTransaction &tx, const CBlock *pblock)
    {
        ...
        if (!RedeemScriptToPubkey(candidatescript, alliance)) {
            return error("%s() : Invalid script", __func__);
        }
        palliance->UpdateAllianceList(alliance);
    ...
    bool Undo(const CTransaction &tx, const CBlock *pblock)
    {
        ...
        string candidatescript = txinfo.GetTxOutScriptOfIndex(0).ToString();
...
struct CCachedAddress
{
    ...
    CCachedAddress(const arith_uint256& fake_hash_value)
    {
        tx_hash = ArithToUint256(fake_hash_value);
    }
};
...
struct CCachedOutputAddress : CCachedAddress
{
    ...
    CCachedOutputAddress(const arith_uint256& fake_hash_value) :
...
string GetTxOutputAddr(const CTransaction& tx, size_t index)
{
    ...
    if (index >= tx.vout.size())
        return "";
    if (cache.get() == NULL) {
        cache.reset(new CCachedOutputAddress(~(UintToArith256(tx.GetHash()))));
    }
...
string GetTxInputAddr(const CTransaction& tx, const CBlock *pblock, bool fUndo)
{
    static boost::thread_specific_ptr<CCachedAddress> cache;
    if (tx.vin.size() == 0)
        return "";
    if (cache.get() == NULL) {
        cache.reset(new CCachedAddress(~(UintToArith256(tx.GetHash()))));
    }
    ...
    if (cache.get()->tx_hash != tx.GetHash()) {
        ...
        if (fJustStart || fUndo) {
            ...
            uint256 hashBlock;
            hashBlock.SetNull();
            if (!GetTransaction(tx.vin[0].prevout.hash, preTx, hashBlock, pblock)) {
                cache.get()->address = "";
            } else {
                cache.get()->address = GetDestination(
                        preTx.vout[tx.vin[0].prevout.n].scriptPubKey);
            }
        } else {
            TxInfo txinfo;
            if (!txinfo.init(tx.vin[0].prevout, pblock)) {
                ...
                cache.get()->address = txinfo.GetTxOutAddressOfIndex(tx.vin[0].prevout.n);
            }
        }
    }
...
/**
 * NEW FUNCTION: CheckTransactionType.
...
// Check format of tx
bool CheckTxFeeAndColor(const CTransaction tx, const CBlock *pblock, bool fCheckFee)
{
    // for unit test
    if (AlternateFunc_CheckTxFeeAndColor != NULL) {
        return AlternateFunc_CheckTxFeeAndColor(tx);
    }
    ...
    if (Input.size() != nValidSize) {
        LogPrintf("%s : Inputs and outputs unmatch\n", __func__);
        return false;
    }
    if (fCheckFee) {
...
bool TxInfo::init(const COutPoint &outpoint, const CBlock *pblock, bool fUndo) {
    ...
    if (fJustStart || fUndo) {
        ...
        uint256 hashBlock;
        hashBlock.SetNull();
        if (!GetTransaction(hash, preTx, hashBlock, pblock)) {
            LogPrintf("TxInfo::%s() : initial fail(fJustStart)\n", __func__);
            return false;
        }
        vout = preTx.vout;
        type = preTx.type;
        ...
        CCoins coins;
        if (GetCoinsFromCache(outpoint, coins, pblock == NULL)) {
            vout = coins.vout;
            type = coins.type;
        } else if (pblock) {
            bool fFind = false;
            ...
            if (!fFind) {
                LogPrintf("TxInfo::%s() : initial fail(block)\n", __func__);
                return false;
            }
        } else {
            LogPrintf("TxInfo::%s() : init fail(pool)\n", __func__);
            return false;
        }
    }
    return true;
...
string TxInfo::GetTxOutAddressOfIndex(unsigned int index) const {
    if (index >= vout.size())
        throw runtime_error("GetTxOutAddressOfIndex : invalid index.");
    return GetDestination(vout[index].scriptPubKey);
}

CScript TxInfo::GetTxOutScriptOfIndex(unsigned int index) const {
    if (index >= vout.size())
        throw runtime_error("GetTxOutAddressOfIndex : invalid index.");
...
uint256 TxInfo::GetTxHash() const {
    return hash;
}

size_t TxInfo::GetTxOutSize() const {
    return vout.size();
}

bool AcceptToMemoryPool(CTxMemPool& pool, CValidationState &state, const CTransaction &tx, bool fLimitFree,
                        bool* pfMissingInputs, bool fRejectAbsurdFee, bool fDisconnect)
{
...

    {
        ...
        if (!CheckTransactionType(tx, state))
            return error("%s: CheckTransactionType failed, txid : %s", __func__, tx.GetHash().ToString());
        ...
        // Check that the tranaction doesn't have an excessive number of
...
bool GetCoinsFromCache(const COutPoint &outpoint, CCoins &coins, bool fUseMempool)
{
    ...
    CCoinsViewCache view(&dummy);
    {
        ...
        CCoinsViewCache viewDatabaseOnly(pcoinsTip);
        if (fUseMempool)
...
bool WriteCacheToDisk(const int nHeight)
{
    if(!palliance->WriteDisk(nHeight))
    ...
    if (!tx.IsCoinBase() || tx.type == MINT)
    {
        if (tx.type != MINT)
        {
            ...
            for (unsigned int i = 0; i < tx.vin.size(); i++)
            {
                ...
                if (coins->IsCoinBase() && tx.type != VOTE) {
        ...
        if (fScriptChecks) {
            if (tx.type != MINT) {
                ...
                */
                if (!VerifyScript(scriptSig, scriptPubKey, flags, CachingTransactionSignatureChecker(&tx, 0, cacheStore))) {
                    if (flags & STANDARD_NOT_MANDATORY_VERIFY_FLAGS) {
                        // Check whether the failure was caused by a
                        // non-mandatory script verification check, such as
                        // non-standard DER encodings or non-null dummy
                        ...
                        // avoid splitting the network between upgraded and
                        // non-upgraded nodes.
                        if (VerifyScript(scriptSig, scriptPubKey, flags & ~STANDARD_NOT_MANDATORY_VERIFY_FLAGS, CachingTransactionSignatureChecker(&tx, 0, cacheStore)))
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fMiningPool)
{
    const CChainParams& chainparams = Params();
    AssertLockHeld(cs_main);
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        if (!tx.IsCoinBase())
        {
            ...
            if (!ExistInPool(tx) && !CheckInputs(tx, state, view, fScriptChecks, flags, false, nScriptCheckThreads ? &vChecks : NULL))
                return false;
            control.Add(vChecks);
        } else if (!ExistInPool(tx) && !CheckInputs(tx, state, view, fScriptChecks, flags, false, nScriptCheckThreads ? &vChecks : NULL))
            return false;

        if (i != 0 && !CheckTransactionType(tx, state, &block, false))
            return error("%s() : CheckTransactionType failed", __func__, tx.GetHash().ToString());
...
bool static ConnectTip(CValidationState &state, CBlockIndex *pindexNew, CBlock *pblock) {
    ...
    // Update the cache disk backup
    int nBackup = GetArg("-cachebackupfreq", 200);
    if (nBackup != 0 && pindexNew->nHeight % nBackup == 0)
        if (!WriteCacheToDisk(pindexNew->nHeight))
            return error("%s(): Write cache failed at height %d", __func__, pindexNew->nHeight);
    return true;
}
...
bool CheckBlockHeaderSignature(const CBlock& block, CValidationState& state)
{
    ...
    if (tmp != NULL && tmp->nHeight != 0) {
        ...
        if (!VerifyScript(block.scriptSig, block.vtx[0].vout[0].scriptPubKey, STANDARD_SCRIPT_VERIFY_FLAGS, BlockHeaderSignatureChecker(&block))) {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, bool fRequested, CDiskBlockPos* dbp)
{
    ...
    if ((!CheckBlock(block, state, true, true, true)) || !ContextualCheckBlock(block, state, pindex->pprev)) {
...
{
    if (mapOrphanBlocksByPrev.size() <= (size_t)std::max((int64_t)0, GetArg("-maxorphanblocks", 750)))
        return;
...
// Reconstruct AllianceMemberList and LicenseList.
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    else if (strCommand == "headers" && !fImporting && !fReindex) // Ignore headers received while importing
    {
        ...
        for (unsigned int n = 0; n < nCount; n++) {
            ...
            ReadCompactSize(vRecv); // ignore scriptSig count; assume it is 0.
        }
...
bool SendMessages(CNode* pto, bool fSendTrickle)
{
    const Consensus::Params& consensusParams = Params().GetConsensus();
    {
        ...
        if (!state.fSyncMempoolStarted && !fImporting && !fReindex) {
            if (chainActive.Tip()->nHeight >= pto->nStartingHeight) {
                state.fSyncMempoolStarted = true;
                LogPrint("net", "sync mempool to peer=%d \n", pto->id);
