...
void WalletTxToJSON(const CWalletTx& wtx, Object& entry)
{
    ...
    entry.push_back(Pair("color", (int64_t)wtx.color));
    Array conflicts;
...
Value setaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            _(__func__) + " \"bitcoinaddress\" \"account\"\n"
...
// Send vote to address, added by YC
Value sendvotetoaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 3)
        throw runtime_error(
            _(__func__) + " \"bitcoinaddress\" ( \"comment\" \"comment-to\" )\n"
            "\nSent a vote transaction to a given address.\n"
            + HelpRequiringPassphrase() +
            "\nArguments:\n"
            "1. \"bitcoinaddress\"  (string, required) The bitcoin address to sendvote to.\n"
    ...
    string strError = pwalletMain->SendVote(address.Get(), wtx);
    if (strError != "")
...
// Send license transaction, added by YC
Value sendlicensetoaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
            _(__func__) + " \"bitcoinaddress\" color ( \"comment\" \"comment-to\" )\n"
            "\nSent a license transaction to a given address.\n"
            + HelpRequiringPassphrase() +
            "\nArguments:\n"
            "1. \"bitcoinaddress\"  (string, required) The bitcoin address to send to.\n"
            "2. \"color\"       (numeric, required) The currency type (color) of the license.\n"
            ...
            "\"transactionid\"  (string) The transaction id.\n"
            "\nExamples:\n"
            + HelpExampleCli("sendlicensetoaddress", "\"1M72Sfpbz1BPpXFHz9m3CdqATR44Jvaydd\"")
            + HelpExampleCli("sendlicensetoaddress", "\"1M72Sfpbz1BPpXFHz9m3CdqATR44Jvaydd\" \"donation\" \"seans outpost\"")
            + HelpExampleRpc("sendlicensetoaddress", "\"1M72Sfpbz1BPpXFHz9m3CdqATR44Jvaydd\", \"donation\", \"seans outpost\"")
        );
    ...
    string strError = pwalletMain->SendLicense(address.Get(), color, wtx);
    if (strError != "")
        throw JSONRPCError(RPC_WALLET_ERROR, strError);
...
Value sendtoaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 5)
        throw runtime_error(
            _(__func__) + " \"bitcoinaddress\" amount color ( \"comment\" \"comment-to\" )\n"
            ...
            "\"transactionid\"  (string) The transaction id.\n"
            "\nExamples:\n"
            + HelpExampleCli("sendtoaddress", "\"1M72Sfpbz1BPpXFHz9m3CdqATR44Jvaydd\" 0.1 0")
            + HelpExampleCli("sendtoaddress", "\"1M72Sfpbz1BPpXFHz9m3CdqATR44Jvaydd\" 0.1 0 \"donation\" \"seans outpost\"")
            + HelpExampleRpc("sendtoaddress", "\"1M72Sfpbz1BPpXFHz9m3CdqATR44Jvaydd\", 0.1, 0, \"donation\", \"seans outpost\"")
        );
    ...
    string strError = pwalletMain->SendMoney(address.Get(), nAmount, color, wtx);
    if (strError != "")
        throw JSONRPCError(RPC_WALLET_ERROR, strError);
...
int64_t GetAccountBalance(CWalletDB& walletdb, const string& strAccount, int nMinDepth, const isminefilter& filter, map<type_Color, int64_t>& color_amount)
{
    ...
    for (map<uint256, CWalletTx>::iterator it = pwalletMain->mapWallet.begin(); it != pwalletMain->mapWallet.end(); ++it) {
        ...
        if (nReceived != 0 && wtx.GetDepthInMainChain() >= nMinDepth) {
            if (color_amount.count(wtx.color) == 0)
                color_amount[wtx.color] = 0;
            color_amount[wtx.color] += nReceived;
        }
        if (color_amount.count(wtx.color) != 0)
            color_amount[wtx.color] -= nSent + nFee;
    }
...
int64_t GetAccountBalance(const string& strAccount, int nMinDepth, const isminefilter& filter, map<type_Color, int64_t>& color_amount)
{
    CWalletDB walletdb(pwalletMain->strWalletFile);
    return GetAccountBalance(walletdb, strAccount, nMinDepth, filter, color_amount);
}

int64_t GetAccountColorBalance(CWalletDB& walletdb, const string& strAccount, const type_Color color, int nMinDepth, const isminefilter& filter)
{
    int64_t nBalance = 0;
    ...
    for (map<uint256, CWalletTx>::iterator it = pwalletMain->mapWallet.begin(); it != pwalletMain->mapWallet.end(); ++it) {
        const CWalletTx& wtx = (*it).second;
        if (!IsFinalTx(wtx) || wtx.GetBlocksToMaturity() > 0 || wtx.GetDepthInMainChain() < 0 || wtx.color != color)
            continue;
...
int64_t GetAccountColorBalance(const string& strAccount, const type_Color color, int nMinDepth, const isminefilter& filter)
{
    CWalletDB walletdb(pwalletMain->strWalletFile);
...
Value getlicenseinfo(const Array& params, bool fHelp)
{
    ...
    map<type_Color, pair<string, int64_t> > color_amount;

    pwalletMain->GetLicenseBalance(color_amount);
...
Value getbalance(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 3)
        throw runtime_error(
            _(__func__) + " ( \"account\" minconf includeWatchonly )\n"
    ...
    map<type_Color, int64_t> color_amount;
        ...
        for (map<uint256, CWalletTx>::iterator it = pwalletMain->mapWallet.begin(); it != pwalletMain->mapWallet.end(); ++it) {
            ...
            if (wtx.GetDepthInMainChain() >= nMinDepth) {
                BOOST_FOREACH(const COutputEntry& r, listReceived) {
                    ...
                    color_amount[wtx.color] += r.amount;
                }
            }
            BOOST_FOREACH(const COutputEntry& s, listSent) {
                ...
                color_amount[wtx.color] -= s.amount;
            }
            if (color_amount.count(wtx.color) == 0) // This should not happen.
                color_amount[wtx.color] = 0;
...
Value getcolorbalance(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 4)
        throw runtime_error(
            ...
            "\nExamples:\n"
            "\nThe total amount in the server across all accounts\n"
            + HelpExampleCli("getcolorbalance", "0") +
            "\nThe total amount in the server across all accounts, with at least 5 confirmations\n"
            + HelpExampleCli("getcolorbalance", "0 \"*\" 6") +
            "\nThe total amount in the default account with at least 1 confirmation\n"
            + HelpExampleCli("getcolorbalance", "0 \"\"") +
            "\nThe total amount in the account named tabby with at least 6 confirmations\n"
            + HelpExampleCli("getcolorbalance", "0 \"tabby\" 6") +
            "\nAs a json rpc call\n"
            + HelpExampleRpc("getcolorbalance", "0, \"tabby\", 6")
        );
    ...
    if (params[1].get_str() == "*") {
        ...
        for (map<uint256, CWalletTx>::iterator it = pwalletMain->mapWallet.begin(); it != pwalletMain->mapWallet.end(); ++it) {
            const CWalletTx& wtx = (*it).second;
            if (!wtx.IsTrusted() || wtx.GetBlocksToMaturity() > 0 || wtx.color != color)
                continue;
    ...
    int64_t nBalance = GetAccountColorBalance(strAccount, color, nMinDepth, filter);
...
Value getaddressbalance(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 2 || params.size() < 1)
        throw runtime_error(
            _(__func__) + " ( \"bitcoin-address\" minconf includeWatchonly )\n"
            "\nArguments:\n"
            "1. \"bitcoin-address\"     (string, required) Bitcoin-address.\n"
            "2. \"minDepth\"     (int, option) minDepth for the tx in blockchain.\n"
            "\nResult:\n"
...
Value getunconfirmedcolorbalance(const Array &params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
                _(__func__) + " color\n"
                "Returns the server's total color unconfirmed balance\n"
                + HelpRequiringPassphrase() +
                "\nArguments:\n"
                "1. \"color\"       (numeric, required) The currency type (color) of the coin.\n"
        );
...
Value movecmd(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 5)
        throw runtime_error(
            _(__func__) + " \"fromaccount\" \"toaccount\" amount ( minconf \"comment\" )\n"
...
Value sendfrom(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 4 || params.size() > 6)
        throw runtime_error(
            _(__func__) + " \"fromaddress\" \"tobitcoinaddress\" amount color ( minconf \"comment\" \"comment-to\" )\n"
    ...
    // Send
    string strError = pwalletMain->SendMoneyFromFixedAddress(strFromAddress, address.Get(), nAmount, color, wtx);
    if (strError != "")
        throw JSONRPCError(RPC_WALLET_ERROR, strError);
...
Value sendmany(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 5)
        throw runtime_error(
            _(__func__) + " \"fromaccount\" {\"address\":amount,...} ( minconf \"comment\" )\n"
            ...
            + HelpExampleCli("sendmany", "\"tabby\" \"{\\\"1D1ZrZNe3JUo7ZycKEYQQiQAWd9y54F4XZ\\\":0.01,\\\"1353tsE8YMTA4EuV7dgUXGjNFf9KpVvKHz\\\":0.02}\" 0") +
            "\nSend two amounts to two different addresses setting the confirmation and comment:\n"
            + HelpExampleCli("sendmany", "\"tabby\" \"{\\\"1D1ZrZNe3JUo7ZycKEYQQiQAWd9y54F4XZ\\\":0.01,\\\"1353tsE8YMTA4EuV7dgUXGjNFf9KpVvKHz\\\":0.02}\" 0 6 \"testing\"") +
            "\nAs a json rpc call\n"
            + HelpExampleRpc("sendmany", "\"tabby\", \"{\\\"1D1ZrZNe3JUo7ZycKEYQQiQAWd9y54F4XZ\\\":0.01,\\\"1353tsE8YMTA4EuV7dgUXGjNFf9KpVvKHz\\\":0.02}\", 0, 6, \"testing\"")
        );
    ...
    // Check funds
    int64_t nBalance = GetAccountColorBalance(strAccount, color, nMinDepth, ISMINE_SPENDABLE);
    ...
    bool fCreated = pwalletMain->CreateTransaction(vecSend, color, wtx, keyChange, nFeeRequired, strFailReason);
    if (!fCreated)
...
Value addmultisigaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3) {
        string msg = _(__func__) + " nrequired [\"key\",...] ( \"account\" )\n"
...
Value listwalletaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 2)
        throw runtime_error(
            _(__func__) + "group-of-address number-of-addresses\n"
    ...
    if(group == "-a") { // list all addresses in wallet
        ...
        for (std::set<CKeyID>::iterator it = keyids.begin(); it != keyids.end(); ++it) {
            if (count >= number)
                break;
            string addr = CBitcoinAddress(*it).ToString();
            if(pwalletMain->mapKeyMetadata[*it].nCreateTime == 1) {
                 a.push_back(addr);
                 count++;
            }
        }
...
Value listaccounts(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 2)
        throw runtime_error(
            _(__func__) + " ( minconf includeWatchonly)\n"
    ...
    map<string, map<type_Color, int64_t> > mapAccountBalances;
    mapAccountBalances.clear();
    ...
    for (map<uint256, CWalletTx>::iterator it = pwalletMain->mapWallet.begin(); it != pwalletMain->mapWallet.end(); ++it) {
        ...
        if (mapAccountBalances.count(strSentAccount) == 0 || mapAccountBalances[strSentAccount].count(wtx.color) == 0)
            mapAccountBalances[strSentAccount][wtx.color] = 0;

        mapAccountBalances[strSentAccount][wtx.color] -= nFee;
        BOOST_FOREACH(const COutputEntry& s, listSent)
            mapAccountBalances[strSentAccount][wtx.color] -= s.amount;
        if (nDepth >= nMinDepth) {
            BOOST_FOREACH(const COutputEntry& r, listReceived)
                if (pwalletMain->mapAddressBook.count(r.destination))
                    mapAccountBalances[pwalletMain->mapAddressBook[r.destination].name][wtx.color] += r.amount;
                else {

                    if (mapAccountBalances.count("") == 0 || mapAccountBalances[""].count(wtx.color) == 0)
                        mapAccountBalances[""][wtx.color] = 0;

                    mapAccountBalances[""][wtx.color] += r.amount;
                }
        }
    }
    ...
    for (CWallet::TxItems::iterator it = txOrdered.begin(); it != txOrdered.end(); ++it) {
        ...
        if (pwtx != 0 && entry != 0) {
            if (mapAccountBalances.count(entry->strAccount) == 0 || mapAccountBalances[entry->strAccount].count(pwtx->color) == 0)
                mapAccountBalances[entry->strAccount][pwtx->color] = 0;
            mapAccountBalances[entry->strAccount][pwtx->color] += entry->nCreditDebit;
        }
    }

    Object ret;
    for (map<string, std::map<type_Color, int64_t> >::const_iterator it1 = mapAccountBalances.begin(); it1 != mapAccountBalances.end(); it1++)
...
Value listsinceblock(const Array& params, bool fHelp)
{
    ...
    if (params.size() > 0) {
        ...
        map<uint256, CBlockIndex*>::iterator it = mapBlockIndex.find(blockId);
...
Value mint(const Array& params, bool fHelp)
{
    ...
    CWalletTx wtx;
    EnsureWalletIsUnlocked();
    string strError = pwalletMain->MintMoney( nAmount, wtx, color);
    if (strError != "")
        throw JSONRPCError(RPC_WALLET_ERROR, strError);
    return wtx.GetHash().GetHex();
}
