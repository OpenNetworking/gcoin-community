...
Value listunspent(const Array& params, bool fHelp)
{
    ...
    bool fColor = false;
    type_Color color_filter = 0;
    if (params.size() > 3) {
        fColor = true;
        color_filter = ColorFromValue(params[3]);
    }
...
Value createrawtransaction(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw std::runtime_error(
            ...
            "\nExamples\n"
            + HelpExampleCli(__func__, "\"[{\\\"txid\\\":\\\"myid\\\",\\\"vout\\\":0}]\" \"[{\\\"address\\\":\\\"target address\\\",\\\"value\\\":1000,\\\"color\\\":0}]\"")
            + HelpExampleRpc(__func__, "\"[{\\\"txid\\\":\\\"myid\\\",\\\"vout\\\":0}]\" \"[{\\\"address\\\":\\\"target address\\\",\\\"value\\\":1000,\\\"color\\\":0}]\"")
        );

    LOCK(cs_main);
    RPCTypeCheck(params, boost::assign::list_of(array_type)(array_type));
    ...
    BOOST_FOREACH(const Value& output, sendTo) {
        ...
        const Value& addrvalue = find_value(o, "address");
    ...
    if (params.size() > 1 && params[1].type() != null_type) {
        ...
        BOOST_FOREACH(Value& p, prevTxs) {
...

            {
                ...
                coins->vout[nOut].mValue = CColorAmount(0, 0); // we don't know the actual output value
            }
