...
const CAmount SEND_TYPE_AMOUNT = COIN;
    ...
    if (fFromLoadWallet) {
        ...
        if (fInsertedNew) {
            ...
            if (!wtxIn.hashBlock.IsNull()) {
                BlockMap::iterator it_mBI = mapBlockIndex.find(wtxIn.hashBlock);
                if (it_mBI != mapBlockIndex.end()) {
                    ...
                    int64_t blocktime = it_mBI->second->GetBlockTime();
...
bool CWalletTx::RelayWalletTransaction()
{
    ...
    // FIXME
    // This check will block the mint transaction
    // from broadcasted
...
CAmount CWallet::GetSendLicenseBalance(const type_Color& color) const
{
    CAmount nTotal = 0;
    {
        LOCK2(cs_main, cs_wallet);
...
CAmount CWallet::GetUnconfirmedColorBalance(const type_Color& color) const
{
    CAmount nTotal = 0;
    {
        LOCK2(cs_main, cs_wallet);
...
void CWallet::AvailableCoinsForType(vector<COutput>& vCoins, const type_Color& send_color, int type, bool fOnlyConfirmed, bool fIncludeZeroValue) const
{
    vCoins.clear();
                ...
                if (!(IsSpent(wtxid, i)) && mine != ISMINE_NO && !IsLockedCoin((*it).first, i) && (pcoin->vout[i].nValue == COIN || fIncludeZeroValue)) {
                    ...
                    if (type == LICENSE) {
                        if (fMintLicense && palliance->IsMember(addr)) {
                                // no send_color license owner, alliance create new license.
...
bool CWallet::SelectCoinsForType(const CAmount& nTargetValue, const type_Color& send_color, int type, set<std::pair<const CWalletTx*, unsigned int> >& setCoinsRet, CAmount& nValueRet) const
{
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true) {
                ...
                BOOST_FOREACH(PAIRTYPE(const CWalletTx*, unsigned int) pcoin, setFeeCoins) {
                    if (setCoins.find(pcoin) != setCoins.end()) {
                        strFailReason = _("Collision funds");
                        return false;
                    }
                    setCoins.insert(pcoin);
                }
...
bool CWallet::HDTopUpKeyPool(unsigned int kpSize)
{
    {
        LOCK(cs_wallet);
        ...
        while (setKeyPool.size() < nTargetSize) {
            ...
            CPubKey newKey;
            string keyChainPath;
            if (!HDGetNextChildPubKey(emptyId, newKey, keyChainPath))
                throw runtime_error(_(__func__)+"can't generate HD child key");
...
bool CWallet::AddKeyPool(CPubKey& key)
{
    LOCK(cs_wallet);
    ...
    if (!setKeyPool.empty())
        nEnd = *(--setKeyPool.end()) + 1;
    if (!walletdb.WritePool(nEnd, CKeyPool(key)))
        throw runtime_error(_(__func__)+"() : writing imported key failed");
    setKeyPool.insert(nEnd);
    LogPrintf("keypool added key %d, size=%u\n", nEnd, setKeyPool.size());
    return true;
}
        ...
        if (pcoin->vin.size() > 0) {
            ...
            BOOST_FOREACH(CTxIn txin, pcoin->vin) {
                ...
                if (!IsMine(txin)) /* If this input isn't mine, ignore it */
                    continue;
                // FIXME: what if txin.prevout.hash not in mapWallet's key, or txin.prevout.n >= vout.size()?
...
string CWallet::MintMoney(const CAmount& nValue, const type_Color& color, CWalletTx& wtxNew)
{
    ...
    if (color != DEFAULT_ADMIN_COLOR) {
        ...
        int64_t num_of_coins = plicense->NumOfCoins(color), num_of_coins_in_pool = 0;
        {
            LOCK(mempool.cs);
    ...
    *static_cast<CTransaction*>(&wtxNew) = CTransaction(txNew);
    if (!CommitTransaction(wtxNew, reservekey))
        return _("Error: The mint transaction was rejected! Read debug.info to get more information.");
    return "";
}
...
int CMerkleTx::GetBlocksToMaturity(const int &type) const
{
